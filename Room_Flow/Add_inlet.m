function [U,V,W] = Add_inlet(U,V,W,Umag,Vmag,Wmag,pos,Size,wall)
...
elseif strcmp(wall,'roof')
    U( horizontal_bounds, vertical_bounds, end) = reshape(Umag, shapeRF) - U(horizontal_bounds, end-1 ,vertical_bounds);
    V( horizontal_bounds, vertical_bounds, end) = reshape(Vmag, shapeRF) - V(horizontal_bounds, end-1 ,vertical_bounds);
    W( horizontal_bounds, vertical_bounds, end) = -reshape(Wmag, shapeRF);
elseif strcmp(wall,'floor')
    U( horizontal_bounds, vertical_bounds, 1) = reshape(Umag, shapeRF) - U(horizontal_bounds, 2 ,vertical_bounds);
    V( horizontal_bounds, vertical_bounds, 1) = reshape(Vmag, shapeRF) - V(horizontal_bounds, 2 ,vertical_bounds);
