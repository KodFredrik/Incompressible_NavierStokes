function  ScalarMovie(Xi,Nx,Ny,Nz,time, gamma, strength)
...
for k=1:Nx
    ...
    colorbar
    colormap jet
    title(axh, ['Concentration at time =' sprintf('%0.2f',time) ', gamma = ' sprintf('%0.8f', gamma) ', Source Strength = ' sprintf('%0.5f',strength)]) 
    xlabel(axh, 'X')
    ylabel(axh, 'Y')
    zlabel(axh, 'Z')
