function [U,V,W] = Add_outlet(U,V,W,Umag,Vmag,Wmag,pos,Size,wall,dx,dy,dz)
...
elseif strcmp(wall,'roof')
    U( horizontal_bounds, vertical_bounds, end) = U(horizontal_bounds, end-1 ,vertical_bounds) + reshape(Umag, shapeRF)*dz;
    V( horizontal_bounds, vertical_bounds, end) = V(horizontal_bounds, end-1 ,vertical_bounds) + reshape(Vmag, shapeRF) *dz;
...
elseif strcmp(wall,'floor')
    U( horizontal_bounds, vertical_bounds, 1) = U(horizontal_bounds, 2 ,vertical_bounds) - reshape(Umag, shapeRF) *dz;
    V( horizontal_bounds, vertical_bounds, 1) = V(horizontal_bounds, 2 ,vertical_bounds) - reshape(Vmag, shapeRF) *dz;
